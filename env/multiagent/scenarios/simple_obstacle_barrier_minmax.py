...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world,seed=None):
        def point_in_triangle2(A, B, C, P):
            ...
            return ~np.logical_and(has_neg, has_pos)
        aa = 0.15
        A = np.array([-0.6 - 1*aa, -0.8-1*aa])
        B = np.array([-0.2+1*aa, -0.9-1*aa])
        C = np.array([-0.5, -0.3+2*aa])
        if seed != None:
            np.random.seed(seed)
        ...
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                ...
                while(flag):
                    ...
                    x1, y1 = 0.25, -0.2
                    width, height = 0.35, 0.5
                    rec_flag = True

                    if x1-world.land_size*1.5 <= landmark.state.p_pos[0] <= x1 + width +world.land_size*1.5 and y1-world.land_size*1.5 <= landmark.state.p_pos[1] <= y1 + height+world.land_size*1.5:
                        rec_flag = False
                    ...
                    elli_flag = True
                    if ((landmark.state.p_pos[0] - h) ** 2 / a ** 2) + ((landmark.state.p_pos[1] - k) ** 2 / b ** 2) <= 1.8:
                        elli_flag = False

                    points = np.array([[landmark.state.p_pos[0], landmark.state.p_pos[1]]])
                    tri_flag = point_in_triangle2(A, B, C, points.T)[0]
                    tri_flag = not tri_flag

                    if tri_flag and rec_flag and elli_flag and count == len(world.obstacles) and not(False in landmark_c):
                        flag = False
        ...
        for agent in world.agents:
            ...
            while(flag):
                ...
                if x1 - world.land_size * 1.5 <= agent.state.p_pos[
                    0] <= x1 + width + world.land_size * 1.5 and y1 - world.land_size * 1.5 <= agent.state.p_pos[
                    1] <= y1 + height + world.land_size * 1.5:
                    rec_flag = False
                ...
                elli_flag = True
                if ((agent.state.p_pos[0] - h) ** 2 / a ** 2) + ((agent.state.p_pos[1] - k) ** 2 / b ** 2) <= 1.8:
                    elli_flag = False

                points = np.array([[agent.state.p_pos[0], agent.state.p_pos[1]]])
                tri_flag = point_in_triangle2(A, B, C, points.T)[0]
                tri_flag = not tri_flag

                if tri_flag and rec_flag and elli_flag and count == len(world.obstacles) and not(False in landmark_c) and not(False in agent_c):
                    flag = False
