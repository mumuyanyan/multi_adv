...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world,seed=None):
        ...
        for i, agent in enumerate(world.agents):
            agent.color = np.array([0.35-i*0.25, 0.85-i*0.25, 0.35-i*0.25]) if not agent.adversary else np.array([0.85, 0.35, 0.35])
            ...
            while(flag):
                ...
                for i, obstacle in enumerate(world.obstacles):
                    if not self.is_collision_aug(obstacle, agent,2.):
                        count = count + 1
                landmark_c = [np.linalg.norm(agent.state.p_pos - landmark_pos[j]) > agent.size * 4 for j in
